...
<c:url value="/ajax/admin/director/edit" var="saveDirectorUrl"></c:url>
<c:url value="/api/admin/director" var="apiDirector"/>
<c:url value="/admin/director/list" var="directorListUrl"></c:url>
<html>
<head>
    <title><fmt:message key="label.director.list" bundle="${lang}"/></title>
</head>
<body>
<div class="breadcrumbs" id="breadcrumbs">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page"><fmt:message key="label.director.list"
                                                                                bundle="${lang}"/></li>
        </ol>
    </nav>
</div>
<div class="card mb-4">
    ...
    <div class="card-header">
        <fmt:message key="label.director.list" bundle="${lang}"/>
        <button type="button" class="float-right btn btn-outline-success"
                onclick="updateDirectorShow('<c:url value="/ajax/admin/director/edit"/>')">
            <fmt:message key="label.director.add" bundle="${lang}"/></button>
    </div>
    <form:form action="" id="form-director" method="get" modelAttribute="items">
        <div class="card-body">
                            ...
                            <fmt:bundle basename="ApplicationResource">
                                <display:table id="tableList" name="items.listResult" partialList="true"
                                    ...
                                    <display:column property="id" titleKey="label.id" sortable="true"
                                                    sortName="id"/>
                                    <display:column property="code" titleKey="label.director.code" sortable="true"
                                                    sortName="code"/>
                                    <display:column property="name" titleKey="label.director.name" sortable="true"
                                                    sortName="name"/>
                                    ...
                                    <display:column titleKey="label.action" class="">
                                        <c:url var="updateDirectorUrl" value="/ajax/admin/director/edit">
                                            <c:param name="id" value="${tableList.id}"/>
                                        </c:url>
                                        <button type="button" class="btn btn-sm btn-primary btn-update"
                                                onclick="updateDirectorShow('${updateDirectorUrl}')"
                                                data-toggle="tooltip"
...
<script>
    $('#select-limit').on("change", function () {
        $('#form-director').submit();
    });

    function updateDirectorShow(url) {
